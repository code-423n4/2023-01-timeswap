...
contract TimeswapV2OptionTest is Test {
    ...
    function assertOutputAndPositionsLong0(uint256 expectedAmount, uint256 outputAmount, uint256 maturity, address userAddress, uint256 strike) internal {
    ...
    function assertOutputAndPositionsLong1(uint256 expectedAmount, uint256 outputAmount, uint256 maturity, address userAddress, uint256 strike) internal {
    ...
    function assertOutputAndPositionsShort(uint256 expectedAmount, uint256 outputAmount, uint256 maturity, address userAddress, uint256 strike) internal {
    ...
    function testMintGivenTokensAndLongs(
        ...
        vm.assume(
            long0To != address(0) && long1To != address(0) && shortTo != address(0) && amount0 != 0 && amount1 != 0 && strike != 0
            // && strike > (1 << 128)
        ...
        if (strike >= type(uint128).max) {
            unchecked {
                vm.assume(isMulDivPossible(amount1, (uint256(1) << 128), strike) && isAddable(amount0, StrikeConversion.convert(amount1, strike, false, false)));
            }
        } else {
            unchecked {
                vm.assume(isMulDivPossible(amount0, strike, (uint256(1) << 128)) && isAddable(amount1, StrikeConversion.convert(amount0, strike, true, false)));
            }
        }
        ...
        assertOutputAndPositionsShort(StrikeConversion.combine(amount0, amount1, res.strike, false), res.short, res.maturity, shortTo, res.strike);
    }
    ...
    function testMintGivenShorts(
        ...
        if (strike >= type(uint128).max) {
            unchecked {
                vm.assume(isAddable(amount0, amount1) && isMulDivPossible(amount1, strike, (1 << 128)));
            }
        } else {
            unchecked {
                vm.assume(isAddable(amount0, amount1) && isMulDivPossible(amount0, (1 << 128), strike));
            }
        }
        ...
        assertOutputAndPositionsLong0(StrikeConversion.turn(amount0, strike, false, true), res.token0, res.maturity, long0To, res.strike);
        assertOutputAndPositionsLong1(StrikeConversion.turn(amount1, strike, true, true), res.token1, res.maturity, long1To, res.strike);
    ...
    function testSwapGivenToken1(
        ...
        console.log(opPair.totalPosition(strike, block.timestamp + 100, TimeswapV2OptionPosition.Long0), opPair.totalPosition(strike, block.timestamp + 100, TimeswapV2OptionPosition.Long1));
        delete param;
    ...
    function testCollectGivenShort(uint256 amount, uint256 amount0, uint256 amount1, address token0To, address token1To, uint256 strike) public {
        vm.assume(strike > 0 && token0To != address(0) && token1To != address(0) && amount1 > 0 && amount0 > 0 && amount > 0);
    ...
    function testCollectGivenToken0(uint256 amount, uint256 amount0, uint256 amount1, address token0To, address token1To, uint256 strike) public {
        vm.assume(strike > 0 && token0To != address(0) && token1To != address(0) && amount1 > 0 && amount0 > 0 && amount > 0 && amount < amount0);
        if (strike >= type(uint128).max) {
            unchecked {
                vm.assume(isMulDivPossible(amount1, (uint256(1) << 128), strike) && isAddable(amount0, StrikeConversion.convert(amount1, strike, false, false)));
            }
        ...
        vm.assume(shortAmount <= opPair.positionOf(strike, block.timestamp + 100, address(this), TimeswapV2OptionPosition.Short) && isMulDivPossible(shortAmount, totalLong1, denominator));
    ...
    function testCollectGivenToken1(uint256 amount, uint256 amount0, uint256 amount1, address token0To, address token1To, uint256 strike) public {
        vm.assume(strike > 0 && token0To != address(0) && token1To != address(0) && amount1 > 0 && amount0 > 0 && amount > 0 && amount < amount1);
        ...
        vm.assume(shortAmount <= opPair.positionOf(strike, block.timestamp + 100, address(this), TimeswapV2OptionPosition.Short) && isMulDivPossible(shortAmount, totalLong0, denominator));
    ...
    function assertOutputAndPositionsBurnt(uint256 mintedAmount0, uint256 mintedAmount1, uint256 mintedShort, uint256 burntAmount0, uint256 burntAmount1, uint256 burntShort) internal {
        assertGe(mintedAmount0, burntAmount0);
        assertGe(mintedAmount1, burntAmount1);
        assertGe(mintedShort, burntShort);
    }

    function testBurnGivenShorts(uint256 strike, uint256 amount0, uint256 amount1, uint256 burnAmount0, uint256 burnAmount1) public {
        ...
        vm.assume(burnAmount1 < amount1 && burnAmount0 < amount0 && burnAmount0 != 0 && burnAmount1 != 0);
        ...
        assertOutputAndPositionsBurnt(mintRes.token0, mintRes.token1, mintRes.short, burnRes.token0AndLong0Amount, burnRes.token1AndLong1Amount, burnRes.shortAmount);
    }

    function testBurnGivenTokensAndLongs(uint256 strike, uint256 amount0, uint256 amount1, uint256 burnAmount0, uint256 burnAmount1) public {
        //vm.assume(amount0 != 0 && amount1 != 0 && strike != 0);
        //vm.assume(amount0 < (1 << 128) && amount1 < (1 << 128));
        vm.assume(
            //long0To != address(0) &&
