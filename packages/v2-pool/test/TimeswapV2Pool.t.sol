...
contract TimeswapV2PoolTest is Test {
    ...
    function testPoolList(uint256 strike, uint256 maturity, uint160 rate, uint256 _chosenTransactionFee, uint256 _chosenProtocolFee) public {
        vm.assume(_chosenTransactionFee <= type(uint16).max && _chosenProtocolFee <= type(uint16).max && maturity > block.timestamp && strike != 0 && rate != 0);
    ...
    function testNumberOfPools(uint256 strike, uint256 maturity, uint160 rate, uint256 _chosenTransactionFee, uint256 _chosenProtocolFee) public {
        vm.assume(_chosenTransactionFee <= type(uint16).max && _chosenProtocolFee <= type(uint16).max && maturity > block.timestamp && strike != 0 && rate != 0);
        chosenTransactionFee = _chosenTransactionFee;
        chosenProtocolFee = _chosenProtocolFee;
        setUp();
    ...
    function testLiquidity(
        ...
        (f.token0, f.token1, f.shorts) = pool.collectProtocolFees(
        ...
        assertGe(f.token1, 0);
        assertGe(f.shorts, 0);
        (f.token0, f.token1, f.shorts) = pool.collectTransactionFees(
            TimeswapV2PoolCollectParam({
                strike: strike,
                maturity: time.maturity,
                long0To: address(this),
    ...
    function testMintGivenShort(uint256 strike, address to, uint256 maturity, uint256 _chosenTransactionFee, uint256 _chosenProtocolFee, uint160 rate, uint256 delta) public {
        vm.assume(
        ...
        TimeswapV2PoolMintParam memory param = TimeswapV2PoolMintParam({strike: strike, maturity: maturity, to: to, transaction: TimeswapV2PoolMint.GivenShort, delta: delta, data: ""});
    ...
    function testMintGivenLiquidity(uint256 strike, address to, uint256 maturity, uint256 _chosenTransactionFee, uint256 _chosenProtocolFee, uint160 rate, uint256 delta) public {
        vm.assume(
            maturity < type(uint96).max &&
                _chosenTransactionFee <= type(uint16).max &&
                _chosenProtocolFee <= type(uint16).max &&
        ...
        TimeswapV2PoolMintParam memory param = TimeswapV2PoolMintParam({strike: strike, maturity: maturity, to: to, transaction: TimeswapV2PoolMint.GivenLiquidity, delta: delta, data: ""});
    ...
    function timeswapV2PoolBurnChoiceCallback(TimeswapV2PoolBurnChoiceCallbackParam calldata param) external returns (uint256 long0Amount, uint256 long1Amount, bytes memory data) {
        ...
        if (param.strike >= type(uint128).max) {
            unchecked {
                vm.assume(isMulDivPossible(long1Amount, (uint256(1) << 128), param.strike) && isAddable(long0Amount, StrikeConversion.convert(long1Amount, param.strike, false, false)));
            }
        } else {
            unchecked {
                vm.assume(isMulDivPossible(long0Amount, param.strike, (uint256(1) << 128)) && isAddable(long1Amount, StrikeConversion.convert(long0Amount, param.strike, true, false)));
            }
        }
        // vm.assume(
    ...
    function timeswapV2PoolDeleverageChoiceCallback(TimeswapV2PoolDeleverageChoiceCallbackParam calldata param) external returns (uint256 long0Amount, uint256 long1Amount, bytes memory data) {
    ...
    function testBurnCheck() public {
        ...
        TimeswapV2PoolMintParam memory param = TimeswapV2PoolMintParam({strike: strike, maturity: maturity, to: address(this), transaction: TimeswapV2PoolMint.GivenLong, delta: delta, data: ""});
        ...
        (burnResponse.liquidityAmount, burnResponse.long0Amount, burnResponse.long1Amount, burnResponse.shortAmount, burnResponse.data) = pool.burn(param2);
        delete param;
